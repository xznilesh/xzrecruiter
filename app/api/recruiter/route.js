...
function statusFor(error){
  ...
  if(['assignment_manager_only','recruiter_workspace_forbidden','requirement_access_forbidden','recruiter_intake_forbidden','candidate_access_forbidden','task_access_forbidden','task_forbidden','recruiter_self_assignment_only'].includes(error))return 403;
  if(['job_not_found','task_not_found'].includes(error))return 404;
...
export async function POST(req){
  ...
  if(action==='saveAssignment'){
    ...
    const result=await recruiterAction('saveAssignment',{
      jobId:body.jobId,recruiterUserId:body.recruiterUserId,dailyTarget:body.dailyTarget,
      status:body.status,priorityContext:String(body.priorityContext||'').slice(0,500),
  ...
  if(action==='setRequirementTarget'){
    ...
    const result=await recruiterAction('setRequirementTarget',{jobId:body.jobId,dailyTarget:body.dailyTarget}).catch(()=>null);
  ...
  if(action==='saveTask'){
    ...
    const result=await recruiterAction('saveTask',{task:{
      ...
      assignedUserId:task.assignedUserId||'',taskType:String(task.taskType||'').slice(0,50),
  ...
  if(action==='setTaskStatus'){
    ...
    const result=await recruiterAction('setTaskStatus',{taskId:body.taskId,status:String(body.status||'').slice(0,30)}).catch(()=>null);
