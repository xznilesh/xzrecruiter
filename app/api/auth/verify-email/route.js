...
export async function POST(req){
  ...
  try{
    const gate=await consumeRateLimit({
      scope:'auth:verify_email',identity:rateLimitIdentityForRequest(req,accessToken.slice(0,32)),limit:20,windowSeconds:900
    });
    if(!gate.allowed)return NextResponse.json({error:'Too many verification attempts.'},{status:429});
  }catch{
