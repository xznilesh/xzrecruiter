...
export async function POST(req) {
  ...
  if (!accessToken) return NextResponse.json({ error: 'Reset proof is missing.' }, { status: 400 });
  ...
  try {
    ...
    const result = await rpc('xzrecruiter_reset_password_verified', { p_new_password: password }, { accessToken });
    if (!result?.ok) return NextResponse.json({ error: 'Could not reset this password.' }, { status: 400 });
