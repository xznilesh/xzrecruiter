...
export default function AppShell({ user, globalSettings, active = 'home', children }) {
  ...
  return <div className="xz-app" dir={direction} data-language={language}>
    ...
    <div className="xz-stage"><header className="xz-topbar"><div className="mobile-brand"><Brand compact /></div><WorkspaceSelector name={workspaceName} countryName={globalSettings?.country_name || 'Global workspace'} role={user?.role} /><button className="global-search" type="button" onClick={() => setCommandOpen(true)} aria-label="Open global command palette"><span aria-hidden="true">⌕</span><span className="global-search-copy">{shellText.commandPlaceholder || 'Search or jump anywhere…'}</span><kbd>⌘ K</kbd></button><div className="topbar-actions"><div className="topbar-pop-wrap"><button className="icon-button" type="button" onClick={() => { setQuickOpen(!quickOpen); setNotifyOpen(false); setUserOpen(false); }} aria-expanded={quickOpen} aria-label="Quick create">＋</button>{quickOpen && <div className="topbar-pop quick-pop" role="menu"><b>Quick actions</b><Link href="/candidates?action=create" onClick={() => setQuickOpen(false)}>Create candidate <span>Open</span></Link><Link href="/jobs?action=create" onClick={() => setQuickOpen(false)}>Create job <span>Open</span></Link><Link href="/clients?action=create" onClick={() => setQuickOpen(false)}>Create account <span>Open</span></Link><Link href="/contacts?action=create" onClick={() => setQuickOpen(false)}>Create contact <span>Open</span></Link><Link href="/opportunities?action=create" onClick={() => setQuickOpen(false)}>Create opportunity <span>Open</span></Link><Link href="/tasks?action=create" onClick={() => setQuickOpen(false)}>Create task <span>Open</span></Link><Link href="/pipeline" onClick={() => setQuickOpen(false)}>Recruitment pipeline <span>Open</span></Link><Link href="/business/pipeline" onClick={() => setQuickOpen(false)}>Business pipeline <span>Open</span></Link><Link href="/interviews?action=create" onClick={() => setQuickOpen(false)}>Schedule interview <span>Open</span></Link></div>}</div><div className="topbar-pop-wrap"><button className="icon-button" type="button" onClick={() => { setNotifyOpen(!notifyOpen); setQuickOpen(false); setUserOpen(false); }} aria-expanded={notifyOpen} aria-label="Notifications">◔</button>{notifyOpen && <div className="topbar-pop notification-pop"><b>Notifications</b><p>You’re all caught up.</p></div>}</div><div className="topbar-pop-wrap"><button className="user-button" type="button" onClick={() => { setUserOpen(!userOpen); setQuickOpen(false); setNotifyOpen(false); }} aria-expanded={userOpen} aria-label="Open user menu"><span>{(user?.display_name || user?.email || 'R').slice(0,1).toUpperCase()}</span></button>{userOpen && <div className="topbar-pop user-pop"><div className="user-pop-head"><b>{user?.display_name || 'Recruiter'}</b><small>{user?.email}</small><span>{user?.role}</span></div><Link href="/settings" onClick={() => setUserOpen(false)}>Configuration Center</Link><Link href="/settings/global" onClick={() => setUserOpen(false)}>Global settings</Link><form action="/api/auth/logout" method="post"><button type="submit">Sign out</button></form></div>}</div></div></header><main className="xz-content">{children}</main></div>
