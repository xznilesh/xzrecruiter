...
export const commandCatalog=[
 {id:'home',label:'Go to Home',keywords:'dashboard home',href:'/dashboard',group:'Navigate',enabled:true},
 {id:'candidates',label:'Open Candidates',keywords:'candidate resume cv people',href:'/candidates',group:'Recruitment',enabled:true},
 {id:'jobs',label:'Open Jobs',keywords:'job requisition vacancy role',href:'/jobs',group:'Recruitment',enabled:true},
 {id:'pipeline',label:'Open Recruitment Pipeline',keywords:'application stage workflow shortlist',href:'/pipeline',group:'Recruitment',enabled:true},
 ...
 {id:'schedule-interview',label:'Schedule interview',keywords:'new interview calendar event',href:'/interviews?action=create',group:'Quick create',enabled:true},
 {id:'agency-setup',label:'Open agency setup',keywords:'onboarding quick advanced agency profile',href:'/onboarding?edit=1',group:'Configure',enabled:true},
 ...
 {id:'global-settings',label:'Open global settings',keywords:'country locale currency timezone language',href:'/settings/global',group:'Configure',enabled:true},
 {id:'csv-import',label:'Import CSV data',keywords:'import company client contact candidate csv',href:'/import',group:'Configure',enabled:true},
...
export default function CommandPalette({open,onClose}){
 const router=useRouter();const[query,setQuery]=useState('');const[active,setActive]=useState(0);const[recentIds,setRecentIds]=useState([]);const inputRef=useRef(null);
 useEffect(()=>{try{setRecentIds(JSON.parse(localStorage.getItem(RECENTS_KEY)||'[]'))}catch{}},[]);
 const items=useMemo(()=>{const q=query.trim().toLowerCase();if(q)return commandCatalog.filter((i)=>`${i.label} ${i.keywords}`.toLowerCase().includes(q));const recent=recentIds.map((id)=>commandCatalog.find((i)=>i.id===id)).filter(Boolean).map((i)=>({...i,group:'Recent'}));return[...recent,...commandCatalog.filter((i)=>!recentIds.includes(i.id))]},[query,recentIds]);
 useEffect(()=>{if(!open)return;setQuery('');setActive(0);const t=setTimeout(()=>inputRef.current?.focus(),20);return()=>clearTimeout(t)},[open]);
 useEffect(()=>{if(active>=items.length)setActive(Math.max(0,items.length-1))},[items.length,active]);
 if(!open)return null;
 function run(item){if(!item?.enabled||!item.href)return;const next=[item.id,...recentIds.filter((id)=>id!==item.id)].slice(0,5);setRecentIds(next);try{localStorage.setItem(RECENTS_KEY,JSON.stringify(next))}catch{}onClose?.();router.push(item.href)}
 function keyDown(e){if(e.key==='Escape'){e.preventDefault();onClose?.()}if(e.key==='ArrowDown'){e.preventDefault();setActive((v)=>Math.min(v+1,items.length-1))}if(e.key==='ArrowUp'){e.preventDefault();setActive((v)=>Math.max(v-1,0))}if(e.key==='Enter'){e.preventDefault();run(items[active])}}
