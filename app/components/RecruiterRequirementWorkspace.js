...
export default function RecruiterRequirementWorkspace({initialContext,jobId}){
 ...
 const[assignment,setAssignment]=useState({recruiterUserId:'',dailyTarget:'1',totalTarget:'0',status:'ACTIVE',priorityContext:'',managerInstructions:''});
 ...
 return <div className="rx-requirement">
   ...
   {canManage?<section className="rx-section rx-manager-controls"><div className="rx-section-head"><div><span className="page-kicker">Manager controls</span><h2>Recruiter assignment & targets</h2><p>Operational assignment only; approved Hiring Brief remains protected.</p></div></div>
     <div className="rx-assignment-grid">
       ...
       <div><h3>Assign recruiter</h3><div className="form-grid two"><label className="form-control"><span>Recruiter</span><select value={assignment.recruiterUserId} onChange={e=>setAssignment({...assignment,recruiterUserId:e.target.value})}><option value="">Choose recruiter</option>{list(ctx.eligible_recruiters).map(r=><option key={r.user_id} value={r.user_id}>{r.display_name||r.email}</option>)}</select></label><label className="form-control"><span>Daily target</span><input type="number" min="0" value={assignment.dailyTarget} onChange={e=>setAssignment({...assignment,dailyTarget:e.target.value})}/></label><label className="form-control"><span>Total target</span><input type="number" min="0" value={assignment.totalTarget} onChange={e=>setAssignment({...assignment,totalTarget:e.target.value})}/></label><label className="form-control"><span>Status</span><select value={assignment.status} onChange={e=>setAssignment({...assignment,status:e.target.value})}>{['ACTIVE','PAUSED','COMPLETED','REMOVED'].map(x=><option key={x}>{x}</option>)}</select></label><label className="form-control"><span>Priority / context</span><input value={assignment.priorityContext} onChange={e=>setAssignment({...assignment,priorityContext:e.target.value})} placeholder="e.g. urgent client SLA"/></label><label className="form-control wide"><span>Manager instructions</span><textarea rows="3" value={assignment.managerInstructions} onChange={e=>setAssignment({...assignment,managerInstructions:e.target.value})}/></label></div><button className="primary-action" onClick={saveAssignment} disabled={!assignment.recruiterUserId}>Save assignment</button></div>
     </div>
     <div className="rx-assignment-list">{list(ctx.assignments).map(a=><div key={a.id}><b>{a.recruiter_name||a.recruiter_user_id}</b><span>{a.assignment_status} · daily {a.daily_target} · total {a.total_submission_target||0}</span><small>{a.manager_instructions||a.priority_context||'No manager instructions'}</small></div>)}</div>
   </section>:null}
