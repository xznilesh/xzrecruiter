...
export default function EnterpriseTable({
  ...
  useEffect(() => {
    try { setSavedViews(JSON.parse(localStorage.getItem(`${storageKey}:views`) || '[]')); } catch {}
  }, [storageKey]);
  ...
  function moveColumn(key, delta) {
    setColumnOrder((order) => {
      const from = order.indexOf(key), to = Math.max(0, Math.min(order.length - 1, from + delta));
  ...
  function applyView(view) {
    setSearch(view.search || ''); setSort(view.sort || { key: '', direction: 'asc' });
    setVisible(view.visible || columns.map((c) => c.key)); setColumnOrder(view.columnOrder || columns.map((c) => c.key)); setPage(1);
  }
  ...
  return <section className="enterprise-table" aria-busy={loading}>
    <div className="table-toolbar">
      <label className="table-search"><span aria-hidden="true">⌕</span><input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search this view" aria-label="Search table" /></label>
      <div className="table-toolbar-actions">
        ...
        <div className="table-view-wrap">
          <button type="button" onClick={() => setShowColumns(!showColumns)}>Columns</button>
          {showColumns && <div className="table-column-menu">
            {columns.map((column) => <div key={column.key} className="column-menu-row">
              <label><input type="checkbox" checked={visible.includes(column.key)} onChange={() => setVisible((v) => v.includes(column.key) ? v.filter((x) => x !== column.key) : [...v, column.key])} /> {column.label}</label>
              <span><button type="button" onClick={() => moveColumn(column.key, -1)} aria-label={`Move ${column.label} left`}>←</button><button type="button" onClick={() => moveColumn(column.key, 1)} aria-label={`Move ${column.label} right`}>→</button></span>
            </div>)}
          </div>}
        </div>
        <button type="button" onClick={saveView}>Save view</button>
        {savedViews.length > 0 && <select aria-label="Saved views" defaultValue="" onChange={(e) => applyView(savedViews.find((v) => v.name === e.target.value) || {})}><option value="" disabled>Saved views</option>{savedViews.map((view) => <option key={view.name}>{view.name}</option>)}</select>}
      </div>
    </div>
    ...
    {error ? <div className="table-state error" role="alert"><b>Couldn’t load this view.</b><span>{error}</span></div> : loading ? <div className="table-skeleton" aria-label="Loading records">{Array.from({ length: 6 }).map((_, i) => <i key={i}/>)}</div> : !pageRows.length ? <div className="table-state empty"><b>{emptyText}</b><span>Adjust filters or create a record when the module is enabled.</span></div> : <div className="table-scroll" role="region" aria-label="Data table" tabIndex="0">
      <table>
        ...
        <tbody>{pageRows.map((row, index) => { const id = String(row[rowKey]); return <tr key={id} tabIndex="0" onKeyDown={(e) => keySelect(e, index)} className={selected.has(id) ? 'selected' : ''}><td><input type="checkbox" checked={selected.has(id)} onChange={() => toggleRow(id)} aria-label={`Select row ${index + 1}`} /></td>{orderedColumns.map((column) => <td key={column.key} data-label={column.label}>{column.render ? column.render(row[column.key], row) : String(row[column.key] ?? '—')}</td>)}{rowActions.length > 0 && <td className="row-actions">{rowActions.map((action) => <button key={action.label} type="button" onClick={() => action.onClick?.(row)}>{action.label}</button>)}</td>}</tr>; })}</tbody>
      </table>
    </div>}
