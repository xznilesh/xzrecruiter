...
export default async function JobsPage({searchParams}){
  ...
  const context=await getJobSearch(String(params?.q||''),filters,limit,(page-1)*limit);
  const pipelines=(onboarding.pipelines||[]).filter((p)=>p.pipeline_kind==='RECRUITMENT'&&p.active!==false);
  return <AppShell user={user} globalSettings={globalContext.settings} active="jobs"><div className="page-heading"><div><span className="page-kicker">Recruitment</span><h1>Jobs</h1><p>Global requisitions with server-side filters, configurable pipelines, salary context and public/private visibility.</p></div><span className="status good">● {pipelines.length} pipeline{pipelines.length===1?'':'s'}</span></div><JobWorkspace context={context||{rows:[],total:0,limit,offset:0}} countries={globalContext.countries||[]} timezones={globalContext.timezones||[]} pipelines={pipelines}/></AppShell>;
}
