...
begin
  ...
  select regexp_replace(split_part(lower(btrim(coalesce(a.source_reference,''))),'?',1),'/+ into v_checksum from public.candidate_documents d where d.id=v_doc and d.agency_id=v_agency;
    ...
    select c.id,
      ...
      (v_source_ref<>'' and exists(
        ...
        where a2.agency_id=v_agency and a2.candidate_id=c.id and a2.archived_at is null
          and regexp_replace(split_part(lower(btrim(coalesce(a2.source_reference,''))),'?',1),'/+
