...
// Branch inheritance / Step-1 + Step-2 source must still be physically present.
for(const p of [
...
// Step-2 globalization remains the only country/timezone registry.
...
// Eight representative agencies include multi-timezone US/Australia, bilingual GCC and Chatham NZ.
...
// Additive schema and tenant ownership.
const ownedTables=[
...
// Verified session and server-side workspace derivation.
...
// Quick/Advanced setup, autosave/resume and completion gate.
...
// Step-2 taxonomies are reused and hierarchy can expand per agency.
...
// Company ICP / recruitment + candidate specialization.
...
// Recruitment & BD pipelines are fully data-driven, persistent and configurable.
...
// Custom fields/layouts/views/assignment/permission foundations.
...
// Business roles, teams, territories.
...
// Presets remain proposals, not schema switches.
...
// CSV parser / mapping / idempotent staged import.
...
assert.ok(imports.includes("status='DUPLICATE'"));
assert.ok(imports.includes("status='INVALID'"));
...
// Settings search / command palette / no fake implemented routes.
...
// Mobile, RTL, keyboard/accessibility static gates.
