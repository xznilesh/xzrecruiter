...
export function matchBand(score,{hardFail=false,hardUnknown=false,confidence=1}={}){
  if(hardFail)return 'Mandatory Requirement Missing';
  if(hardUnknown||confidence<0.45)return 'Possible Match';
...
export function computeCandidateMatch({criteria=[],brief={},profile={},weights=BASELINE_WEIGHTS}={}){
  ...
  const recommendation=hardFail||hardUnknown?'RECRUITER_REVIEW_REQUIRED':hardWarn?'REVIEW_AND_SCREEN_IF_RELEVANT':score>=80?'WORTH_SCREENING':score>=55?'REVIEW_AND_SCREEN_IF_RELEVANT':'RECRUITER_REVIEW_REQUIRED';
  return {score,band:matchBand(score,{hardFail,hardUnknown,confidence}),confidence,coverage:Math.round(coverage*100)/100,hardRuleStatus:hardFail?'FAIL':hardUnknown?'UNKNOWN':hardWarn?'WARN':'PASS',hardRules,requirementResults,components,strengths:strengths.slice(0,20),gaps:gaps.slice(0,20),uncertainties:uncertainties.slice(0,30),evidenceMeta:{evaluatedWeight:evaluated,totalWeight:100,scoreIsDecision:false,effectiveWeights:w,profileSchemaVersion:CANDIDATE_PROFILE_SCHEMA_VERSION,matchSchemaVersion:CANDIDATE_MATCH_SCHEMA_VERSION},recommendation};
}
