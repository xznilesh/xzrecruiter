...
export function buildCandidateProfileSnapshot({candidate={},parseRun={},aiExtraction={},resumeText=''}={}){
  ...
  const name=candidate.full_name||candidate.fullName||ai?.identity?.name||extracted.fullName||'';
  const email=candidate.email||ai?.identity?.email||extracted.email||'';
  const phone=candidate.phone||ai?.identity?.phone||extracted.phone||'';
  const title=candidate.current_title||candidate.currentTitle||ai?.professional?.currentTitle||extracted.currentTitle||extracted.headline||'';
  const company=candidate.current_company||candidate.currentCompany||ai?.professional?.currentCompany||'';
  const location=[candidate.city||ai?.identity?.city||'',candidate.region||ai?.identity?.region||'',candidate.country_code||ai?.identity?.countryCode||''].filter(Boolean).join(', ');
  const timeline=Array.isArray(ai?.professional?.employmentTimeline)?ai.professional.employmentTimeline:[];
  const totalExp=num(candidate.experience_years??candidate.experienceYears??ai?.professional?.totalExperienceYears??extracted.experienceYears??calculateExperienceYears(timeline));
  const relevantExp=num(candidate.relevant_experience_years??candidate.relevantExperienceYears??ai?.professional?.relevantExperienceYears);
  ...
  const availability=txt(candidate.availability_status||candidate.availabilityStatus||ai?.availability?.status||'UNKNOWN',80).toUpperCase();
  const notice=num(candidate.notice_period_days??candidate.noticePeriodDays??ai?.availability?.noticePeriodDays);
  const availabilityDate=txt(ai?.availability?.availabilityDate||'',80);
  const workModel=txt(candidate.workplace_preference||candidate.workplacePreference||ai?.workContext?.workModel||'',80).toUpperCase();
  const rawResume=String(resumeText||parseRun?.extracted_text||'');
  const injection=detectCandidatePromptInjectionSignals(rawResume);
  return {
    identity:{
      name:f(name,low(name),name?(confidence.fullName??0.95):0,evidence.fullName?[evidence.fullName]:[],candidate.full_name?'PROFILE':ai?.identity?.name?'AI_EXTRACTION':'RESUME_PARSE','NORMALIZED'),
      email:f(email,normalizeEmail(email),email?(confidence.email??0.99):0,evidence.email?[evidence.email]:[],candidate.email?'PROFILE':ai?.identity?.email?'AI_EXTRACTION':'RESUME_PARSE','NORMALIZED'),
      phone:f(phone,normalizePhone(phone),phone?(confidence.phone??0.9):0,evidence.phone?[evidence.phone]:[],candidate.phone?'PROFILE':ai?.identity?.phone?'AI_EXTRACTION':'RESUME_PARSE','NORMALIZED'),
      location:f(location,normalizeLocation(location),location?0.8:0,ai?.identity?.locationEvidence||[],candidate.city||candidate.country_code?'PROFILE':ai?.identity?.city?'AI_EXTRACTION':'UNKNOWN','NORMALIZED'),
    },
    professional:{
      currentTitle:f(title,normalizeTitle(title),title?(confidence.currentTitle??0.8):0,evidence.currentTitle?[evidence.currentTitle]:[],candidate.current_title?'PROFILE':ai?.professional?.currentTitle?'AI_EXTRACTION':'RESUME_PARSE','NORMALIZED'),
      currentCompany:f(company,low(company),company?0.8:0,ai?.professional?.currentCompanyEvidence||[],candidate.current_company?'PROFILE':ai?.professional?.currentCompany?'AI_EXTRACTION':'UNKNOWN','NORMALIZED'),
      previousTitles,totalExperienceYears:f(totalExp??'',totalExp??'',totalExp!=null?(confidence.experienceYears??0.75):0,evidence.experienceYears?[evidence.experienceYears]:[],candidate.experience_years!=null?'PROFILE':ai?.professional?.totalExperienceYears!=null?'AI_EXTRACTION':'RESUME_PARSE','CALCULATED_OR_NORMALIZED'),
      relevantExperienceYears:f(relevantExp??'',relevantExp??'',relevantExp!=null?0.7:0,ai?.professional?.relevantExperienceEvidence||[],candidate.relevant_experience_years!=null?'PROFILE':ai?.professional?.relevantExperienceYears!=null?'AI_EXTRACTION':'UNKNOWN','CALCULATED_OR_NORMALIZED'),
      employmentTimeline:timeline.slice(0,50),industries,
    },
    skills,certifications:certs,education,
    availability:{
      status:f(availability,availability,availability&&availability!=='UNKNOWN'?0.9:0.2,[],candidate.availability_status?'PROFILE':ai?.availability?.status?'AI_EXTRACTION':'UNKNOWN'),
      noticePeriodDays:f(notice??'',notice??'',notice!=null?0.9:0,[],candidate.notice_period_days!=null?'PROFILE':ai?.availability?.noticePeriodDays!=null?'AI_EXTRACTION':'UNKNOWN'),
      availabilityDate:f(availabilityDate,availabilityDate,availabilityDate?0.8:0,[],availabilityDate?'AI_EXTRACTION':'UNKNOWN'),
    },
    workContext:{
      workplacePreference:f(workModel,workModel,workModel?0.9:0,[],candidate.workplace_preference?'PROFILE':ai?.workContext?.workModel?'AI_EXTRACTION':'UNKNOWN'),
...
export function evaluateHardRule(rule,profile){
  ...
  if(/skill|technology|tool/.test(key)){
    const pieces=requirement.split(/[,/|;+]|\band\b/i).map(x=>x.trim()).filter(Boolean),hits=pieces.map(x=>skillHit(profile,x)).filter(Boolean);
    ...
    return {...base,status:'FAIL',reason:'No candidate evidence was found for the explicitly required skill(s).',confidence:0.8};
  }
...
export function computeCandidateMatch({criteria=[],brief={},profile={},weights=BASELINE_WEIGHTS}={}){
  ...
  const components={mandatorySkills:ratio(mustHits.length,must.length,w.mandatorySkills,mustHits.flatMap(x=>x.evidence))};
  ...
  components.preferredSkills=ratio(niceHits.length,nice.length,w.preferredSkills,niceHits.flatMap(x=>x.evidence));
  ...
  return {score,band:matchBand(score,{hardFail,confidence}),confidence,coverage:Math.round(coverage*100)/100,hardRuleStatus:hardFail?'FAIL':hardUnknown?'UNKNOWN':hardRules.some(x=>x.status==='WARN')?'WARN':'PASS',hardRules,components,strengths:strengths.slice(0,20),gaps:gaps.slice(0,20),uncertainties:uncertainties.slice(0,30),evidenceMeta:{evaluatedWeight:evaluated,totalWeight:100,scoreIsDecision:false},recommendation:hardFail?'RECRUITER_REVIEW_REQUIRED':score>=80?'WORTH_SCREENING':score>=55?'REVIEW_AND_SCREEN_IF_RELEVANT':'RECRUITER_REVIEW_REQUIRED'};
}
