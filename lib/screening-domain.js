...
const VERIFIABLE_FACT_SOURCES = new Set(['RECRUITER_VERIFIED','DOCUMENT_VERIFIED']);
...
export function validateFactAssertion(fact={}) {
  ...
  if(fact.verified===true && !VERIFIABLE_FACT_SOURCES.has(source)) return {ok:false,error:'verification_requires_human_or_document_source'};
  return {ok:true,source,verified:VERIFIABLE_FACT_SOURCES.has(source) && fact.verified===true};
}
...
export function qualificationChecklist(input={}) {
  ...
  add('step4_hard_rules','Step-4 hard-rule status',hardRule,['FAIL','UNKNOWN'].includes(hardRule)&&input.nonOverridable===true);
