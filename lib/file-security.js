...
export function assertSafeStoragePath(value){
  ...
  if(decoded.split('/').some(part=>part==='..'||part==='.')||decoded.includes('\\'))throw new Error('unsafe_storage_path');
  return path;
}
...
export function validatePrivateUpload({bytes,mimeType,filename='',storagePath='',sizeBytes=null,maxBytes=MAX_PRIVATE_UPLOAD_BYTES}){
