...
export function formatSalary({ min, max, amount, currency, period = 'ANNUAL', locale = 'en-GB', grossNet = 'UNSPECIFIED' }) {
  ...
  if (high !== null && high !== undefined && Number(high) !== Number(low)) {
    rendered = `${rendered} – ${formatCurrency(high, currency, locale)}`;
  }
...
export function localePreview(profile, amount = 120000) {
  ...
  return {
    date: formatDate('2026-09-03T12:30:00Z', { locale: profile.locale, timezone: profile.timezone }),
