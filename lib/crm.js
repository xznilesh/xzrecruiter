...
export async function getBusinessPipeline(limit=500){
  const token=await sessionToken(); if(!token) return null;
  const result=await rpc('xzrecruiter_business_pipeline_context',{p_token:token,p_limit:limit});
  return result?.ok?result:null;
}

export async function getVendorContext(query='',limit=50,offset=0){
  const token=await sessionToken(); if(!token) return null;
  const result=await rpc('xzrecruiter_vendor_context',{p_token:token,p_query:query,p_limit:limit,p_offset:offset});
  return result?.ok?result:null;
}
...
export async function crmAction(name,args={}){
  ...
  const map={
    ...
    issueClientPortal:['xzrecruiter_issue_client_portal_access',{p_token:token,p_client_id:args.clientId,p_contact_id:args.contactId||null}],
    saveVendor:['xzrecruiter_save_vendor',{p_token:token,p_vendor:args}],
    issueVendorPortal:['xzrecruiter_issue_vendor_portal_access',{p_token:token,p_vendor_id:args.vendorId}],
    shareJobVendor:['xzrecruiter_share_job_with_vendor',{p_token:token,p_vendor_id:args.vendorId,p_job_id:args.jobId,p_active:args.active!==false}]
  };
