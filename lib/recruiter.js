...
export async function getRecruiterHome(limit=50){
  ...
  const result=await rpc('xzrecruiter_recruiter_home',{p_token:token,p_limit:limit});
...
export async function getRecruiterRequirement(jobId,limit=50){
  const token=await sessionToken();
  if(!token)return null;
  const result=await rpc('xzrecruiter_recruiter_requirement_context',{p_token:token,p_job_id:jobId,p_queue_limit:limit});
  return result?.ok?result:null;
}
...
export async function recruiterAction(name,args={}){
  ...
  const map={
    saveAssignment:['xzrecruiter_save_requirement_assignment',{
      ...
      p_daily_target:Number(args.dailyTarget||0),p_status:args.status||'ACTIVE',
      p_priority_context:args.priorityContext||null,p_manager_instructions:args.managerInstructions||null,
    ...
    setTaskStatus:['xzrecruiter_set_execution_task_status',{
      p_token:token,p_task_id:args.taskId,p_status:args.status
    }],
