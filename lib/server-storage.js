...
function encodedPath(path){
  return assertSafeStoragePath(path).split('/').map(encodeURIComponent).join('/');
}

function objectUrl(bucket,path){
  return `${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(safeBucket(bucket))}/${encodedPath(path)}`;
}
...
export async function createSignedPrivateUrl(path,expiresIn=60,bucket=RECRUITMENT_BUCKET){
  if(!storageConfigured())throw new Error('storage_not_configured');
  const safePath=assertSafeStoragePath(path);
  const ttl=Math.max(30,Math.min(Number(expiresIn||60),60));
